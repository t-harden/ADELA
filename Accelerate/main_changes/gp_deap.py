...
def ExtractOpEmbed(pipe):
    ...
    op_embed_list = [dict_operator_light_PreEmbedding[op_str] for op_str in op_str_list]

    return op_embed_list

@threading_timeoutable(default="Timeout")
def score_by_surrogate_acc(sklearn_pipeline, sklearn_pipeline_acc, performance_acc, dataset_embed, suaccModel, use_dask=False):
    ...
    if use_dask:
        ...
        try:
            "dataset embedding"
            da_embed = dataset_embed[None, ...]
            da_embed = torch.from_numpy(da_embed).float()
            "evaluation pipeline embedding"
            op_embed_list = ExtractOpEmbed(sklearn_pipeline)
            ops_embed = np.vstack(op_embed_list)
            ops_embed = torch.from_numpy(ops_embed).float()
            padded_ops_embed = torch.cat((ops_embed, torch.zeros(12 - ops_embed.size(0), ops_embed.size(1))), 0)
            padded_ops_embed = padded_ops_embed[None, :]
            ops_mask = torch.tensor([len(op_embed_list)])
            "accompanying pipeline embedding"
            acc_op_embed_list = ExtractOpEmbed(sklearn_pipeline_acc)
            acc_ops_embed = np.vstack(acc_op_embed_list)
            acc_ops_embed = torch.from_numpy(acc_ops_embed).float()
            acc_padded_ops_embed = torch.cat((acc_ops_embed, torch.zeros(12 - acc_ops_embed.size(0), acc_ops_embed.size(1))), 0)
            acc_padded_ops_embed = acc_padded_ops_embed[None, :]
            acc_ops_mask = torch.tensor([len(acc_op_embed_list)])
            "accompanying pipeline performance"
            performance_acc_embed = np.array([performance_acc])
            performance_acc_embed = performance_acc_embed[None, ...]
            performance_acc_embed = torch.from_numpy(performance_acc_embed).float()
            "prediction"
            out = suaccModel(da_embed, padded_ops_embed, ops_mask, acc_padded_ops_embed, acc_ops_mask, performance_acc_embed)
            out = out.detach().numpy().tolist()
            predicted_cv_score = out[0][0]
            if not (0 <= predicted_cv_score <= 1):
                raise ValueError("Incorrect output format from SurrogateAcc's prediction!")
...
def compute_sklearn_pipe_sim(sklearn_pipe1, sklearn_pipe2):
    "operators嵌入"
    op1_embed1, op2_embed1, op3_embed1 = ExtractOpEmbed(sklearn_pipe1)
    op1_embed2, op2_embed2, op3_embed2 = ExtractOpEmbed(sklearn_pipe2)
    sim_ops = []
    for vec1, vec2 in [(op1_embed1, op1_embed2), (op2_embed1, op2_embed2), (op3_embed1, op3_embed2)]:
        sim_ops.append( vec1.dot(vec2) / (np.linalg.norm(vec1) * np.linalg.norm(vec2)) )
    sim_pipe = np.nanmean(sim_ops)
    return sim_pipe
