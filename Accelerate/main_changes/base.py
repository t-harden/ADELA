...
from .gp_deap import (
    ...
    cxOnePoint,
    score_by_surrogate_acc,
    compute_sklearn_pipe_sim,
)

import torch
from .surrogate_models.AccSurrogateNetDef import AccSurrogateNet_GRU_Residual
...
class TPOTBase(BaseEstimator):
    ...
    def __init__(
        ...
        surrogate_accompany_model=None,

        origin_cv_generations=[],
    ):
        ...
        self.dataset_embed = dataset_embed

        self.surrogate_accompany_model = surrogate_accompany_model
        " Initialize accompanying pipeline list"
        self.accompany_pipe_list = [[0, 0, 0, 0] for _ in range(self.generations + 1)] if self.surrogate_accompany_model is not None \
            else None

        self.origin_cv_generations = origin_cv_generations
    ...
    def _setup_toolbox(self):
        ...
        pset_value = self._pre_pset if self.pre_config_dict else self._pset
    ...
    def fit(self, features, target, sample_weight=None, groups=None):
        ...
        if self.surrogate_accompany_model is not None:
            if self.template is not None:
                SuAccModel = AccSurrogateNet_GRU_Residual("training_parameters")
                SuAccModel.load_state_dict(torch.load("trained_model.pt"))
                SuAccModel.eval()
        else:
            SuAccModel = None
        ...
        self._toolbox.register(
            ...
            groups=groups,
            suaccModel=SuAccModel
        )

        # assign population, self._pop can only be not None if warm_start is enabled
        if not self._pop:
            if not self.customized_pre_population: #If self.customized_pre_population is None，using the original way to generate initial population
                self._pop = self._toolbox.population(n=self.population_size)
                # print(self._pop)
                print("Initial population size：", len(self._pop))
                print("Original way done！")
            else:
    ...
    def _evaluate_individuals(
        self, population, features, target, sample_weight=None, groups=None, suaccModel=None,
    ):
        ...
        if self.surrogate_accompany_model is not None:
            if self.template is not None:
                partial_score_by_surrogate_acc = partial(
                    score_by_surrogate_acc,
        ...
        current_gene = stats_dicts[eval_individuals_str[0]]['generation']
        result_score_list = []
        ...
        try:
            ...
            if self._n_jobs == 1 and not self.use_dask:
                for sklearn_pipeline in sklearn_pipeline_list:
                    self._stop_by_max_time_mins()
                    if self.surrogate_accompany_model is not None:
                        if (current_gene != 0) and (current_gene not in self.origin_cv_generations):
                            max_allpredicted_pipe = max(self.accompany_pipe_list[0:current_gene], key=lambda temp_list: temp_list[2])
                            acc_pipe = max_allpredicted_pipe
                            val = partial_score_by_surrogate_acc(
                                  sklearn_pipeline=sklearn_pipeline,
                                  sklearn_pipeline_acc=acc_pipe[0],
                                  performance_acc=acc_pipe[3]
                                )
                        else:
                    ...
                    if self.use_dask:
                        ...
                        if self.surrogate_accompany_model is not None:
                            if (current_gene != 0) and (current_gene not in self.origin_cv_generations):
                                ...
                                acc_pipe = max_allpredicted_pipe
                            tmp_result_scores = [
                                partial_score_by_surrogate_acc(
                                    sklearn_pipeline=sklearn_pipeline,
                                    sklearn_pipeline_acc=acc_pipe[0],
                                    performance_acc=acc_pipe[3]
                                ) if (current_gene != 0) and (current_gene not in self.origin_cv_generations)
                                else partial_wrapped_cross_val_score(sklearn_pipeline=sklearn_pipeline)
                        ...
                        if self.surrogate_accompany_model is not None:
                            ...
                            tmp_result_scores = parallel(
                                delayed(partial_score_by_surrogate_acc)(
                                    sklearn_pipeline=sklearn_pipeline,
                                    sklearn_pipeline_acc=acc_pipe[0],
                                    performance_acc=acc_pipe[3]
                                ) if (current_gene != 0) and (current_gene not in self.origin_cv_generations)
                                else delayed(partial_wrapped_cross_val_score)(sklearn_pipeline=sklearn_pipeline)
                                for sklearn_pipeline in sklearn_pipeline_list[chunk_idx: chunk_idx + chunk_size]
                            )
                        else:
                            tmp_result_scores = parallel(
            ...
            self._update_evaluated_individuals_(
                result_score_list, eval_individuals_str[:num_eval_ind], operator_counts, stats_dicts,
            )

            if self.surrogate_accompany_model is not None:  # Update self.accompany_pipe_list
                for sklearn_pipeline, ind_str, result_score in zip(sklearn_pipeline_list, eval_individuals_str, result_score_list):
                    if result_score > self.accompany_pipe_list[current_gene][2]:
                        self.accompany_pipe_list[current_gene] = [sklearn_pipeline, ind_str, result_score, 0]
                if (current_gene != 0) and (current_gene not in self.origin_cv_generations):
                    true_cv_score = partial_wrapped_cross_val_score(sklearn_pipeline=self.accompany_pipe_list[current_gene][0])
                    self.accompany_pipe_list[current_gene][3] = true_cv_score
                else:
                    self.accompany_pipe_list[current_gene][3] = self.accompany_pipe_list[current_gene][2]
    ...
    def _initPopulation_custom(self, customized_pre_population):
        iniPop = []
        for deap_pipeline_str in customized_pre_population:
            deap_pipeline = creator.Individual.from_string(deap_pipeline_str, self._pset)
        ...
        if len(iniPop) < self.population_size:
            ...
            self.population_size = len(customized_pre_population)
        return iniPop

    def pipe_sim(self, str_pipe1, str_pipe2):
        deap_pipeline1 = creator.Individual.from_string(str_pipe1, self._pset)
        sklearn_pipeline1 = self._toolbox.compile(expr=deap_pipeline1)
        deap_pipeline2 = creator.Individual.from_string(str_pipe2, self._pset)
        sklearn_pipeline2 = self._toolbox.compile(expr=deap_pipeline2)
        print(sklearn_pipeline1)
        return len(list(sklearn_pipeline1.named_steps.items()))

        sim_pipe = compute_sklearn_pipe_sim(sklearn_pipeline1, sklearn_pipeline2)
        return sim_pipe
